...
for cell_index in cells:
    ...
    #output
    Output = mat_contents['Output_tr_MMMSE_maxprod_cell_' + str(cell_index)]
    ...
    model.save('saved_neural_networks/NN_MMMSE_maxprod_cell_'+ str(cell_index) +'.h5')
    ...
    # Load data
    Input_test = mat_contents['Input_tr_normalized']

    # Run the neural network
    output_NN = model.predict(np.transpose(Input_test))
    print(output_NN)
    # Save output in a matlab file
    sio.savemat('../../matlab_code/data_output/multi_cell/pow_MMMSE_maxprod_cell_'+ str(cell_index) +'.mat', {'Output_test_' + str(cell_index): np.transpose(output_NN)})
